...
## Review
::: incremental
- A random variable $X$ is a function that maps outcomes of a random experiment to real numbers. Formally: $X: \Omega \to \mathbb{R}$. For each $\omega \in \Omega$, $X(\omega) \in\mathbb{R}$.
...
- Compute derived quantities such event probilities e.g., $\P(\theta > 0.5)$, posterior predictive distribution $f(\widetilde{y} \mid y)$, expected utilities $\E(U(x) \mid d)$, etc.
:::
...
- Game Setup:
  ...
  - You draw 10 balls with replacement and observe 6 red balls
...
::: {.fragment}
- Your options are: A) Don't play; B) Pay \$2 to play immediately; C) Pay \$1 to buy an extra ball, then pay \$2 to play
:::
...
::: incremental
- We will assume a uniform prior on $\theta$  
- What does that mean? Is that a reasonable assumption?
- We take a ball from the box; what is $\P(y = \text{red})$?
- LOTP, marginal probability of data, prior predictive probability
:::
...
## Data Model: Before Making Observations {.smaller}
...
#| echo: true
N = 10; y <- 0:N
f_y <- dbinom(x = y, size = N, prob = 1/3)
sum(f_y)
```
:::
...
::: incremental
- We know we pulled out 6 red balls
...
::: incremental
  ...
  - For $\theta = 0.50$: 
    $$
    f(y | 0.50) = \binom{10}{6} (0.5)^{5} (0.5)^{5} \approx 0.21
    $$
  - For $\theta = 0.75$: 
    $$
...
::: {.fragment}
- Let $X$ be the payoff. You want to choose your $\theta$ such that $\E(X)$ is maximized: $\textrm{arg max}_{\theta} \ \mathbb{E}[X \mid \theta]$

$$
\E(X) = f(\theta \mid y) \times \text{Prize } - \text{Cost} 
$$
:::

::: incremental

- Bet on $\theta=0.25$: $0.044 \times 3 \approx 0.132$
- Bet on $\theta=0.50$: $0.559 \times 5 \approx 2.795$: MLE
- Bet on $\theta=0.75$: $0.397 \times 15 \approx 5.955$

- **Best choice:** Bet on $\theta=0.75$ with expected prize \$5.96.

- **Net Expected Payoff:** $\E(X) = 5.96 - 2 = \$3.96$
...
## Option 2: Should You Buy and Extra Ball? {.small}

::: incremental
- Compute the probability of the red ball on the next draw
- Note that prior, unconditional probability of the red ball is 0.50. Why?
- In symbols: $f(\text{red} \mid \theta)$, where $\theta$ has been updated from $1/3$ to $f(\theta \mid y)$

- Posterior predictive probability of red:

  $$
  \begin{eqnarray*}
  \P(\tilde{y} = \text{red} \mid y) &=& 0.044\times 0.25 + 0.559\times 0.5 + 0.397\times 0.75 \\
  &\approx& 0.589
  \end{eqnarray*}
  $$

- Posterior predictive probability of green:
  $$
   \P(\text{green}) = 1 - \P(\text{red})  \approx 0.411
  $$
:::
