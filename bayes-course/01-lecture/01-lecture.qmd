...
[What is your first name and where were you born?](https://www.polleverywhere.com/free_text_polls/uv28f9hf0xrsysKjRNOVU)
...
![](images/nofheads.png){fig-align="center" height="450"}
:::
...
- At the end of the trial, $Y = y \in \{0,1,...,5 \}$ people will have responded[^1] to the treatment
...
::: incremental
- Before observing the data, we need to specify a prior model $f(\theta)$ on all unknowns $\theta$[^2] 
- Pick a data model $f(y \mid \theta, x)$ --- this is typically more important than a prior; this includes the model for the conditional mean: $\E(y | x)$
...
- After we observe data $y$, we treat $f(y \mid \theta, x)$ as the likelihood of observing $y$ under all plausible values of $\theta$
...
[^2]: There is always a prior, even in frequentist inference. 
