...
dot_plot <- function(x, y) {
  ...
  p + geom_point(aes(x = x, y = y), size = 0.5) +
    geom_segment(aes(x = x, y = 0, xend = x, yend = y), size = 0.2) +
    xlab(expression(theta)) + ylab(expression(f(theta)))
}

plot_binom_lik <- function(theta, N, y) {
...
::: incremental
- Before observing the data, we need to speficy a prior model $f(\theta)$ on all uknowns $\theta$[^2] 
...
[^2]: There is alwyas a prior, even in frequentist inference. 
...
# for (i in 1:(N + 1)) {
#   p1[[i]] <- plot_binom_lik(theta, N, y[i]) + xlab(expression(theta)) + ylab("Lik") +
...
## Continuous Version of Bayes's Rule {.smaller}
