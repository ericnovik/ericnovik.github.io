...
::: incremental
- The integral of the likelihood over $\theta$ (under a uniform prior) gives us the prior predictive distribution:
$$
...
$$
\begin{eqnarray*}
f(\text{red} \mid y) &=& \sum_{i=1}^{3} f(\text{red}, \ \theta_i \mid y) = \sum_{i=1}^{3} f(\text{red} \mid \theta_i, \ y) f(\theta_i \mid y) \\
&=& \sum_{i=1}^{3} f(\text{red} \mid \theta_i) f(\theta_i \mid y) \\
 f(\text{red} \mid y) &=& 0.25 \times 0.526  + 0.50 \times 0.416  + 0.75 \times 0.058  \\
  &\approx& 0.383
\end{eqnarray*}
$$
:::
...
## Updating Beliefs: Case 1 — Extra Ball is Red {.smaller}
...
::: {.fragment}
$$
f(\theta_i \mid y, \text{red}) = 
\frac{\overbrace{\theta_i}^{\text{Lik of red}} \cdot \overbrace{f(\theta_i \mid y)}^{\text{Current posterior}}}
{\underbrace{\sum_{j=1}^{3} \theta_j \cdot f(\theta_j | y)}_{f(\text{red}|y) = 0.383}}
$$
:::

## Exercise: Case 1 — Extra Ball is Red {.smaller}
...
## Updating Beliefs: Case 1 — Solution {.tiny}
...
## Updating Beliefs: Case 2 — Extra Ball is Green {.smaller}

By the same logic, with $(1 - \theta_i)$ as the likelihood of green:

::: {.fragment}
$$
f(\theta_i \mid y, \text{green}) = \frac{f(\text{green} \mid \theta_i) \cdot f(\theta_i \mid y)}{f(\text{green} \mid y)} = 
\frac{\overbrace{(1 - \theta_i)}^{\text{Lik of green}} \cdot \overbrace{f(\theta_i \mid y)}^{\text{Current posterior}}}
{\underbrace{\sum_{j=1}^{3} (1 - \theta_j) \cdot f(\theta_j | y)}_{f(\text{green}|y) = 0.617}}
$$
:::

## Exercise: Case 2 — Extra Ball is Green {.smaller}
...
## Updating Beliefs: Case 2 — Solution {.tiny}
...
- **Combine the two cases:**
  ...
  $$
  \begin{align*}
  \E_\text{extra ball}({\text{payoff}}) =& f(\text{red} \mid y)\times 8.15 + f(\text{green} \mid y)\times 6.39 \\
  =& 0.383 \times 8.15 + 0.617 \times 6.39 \approx 7.06
  \end{align*}
  $$
