...
::: incremental
- Last time we presented the discritized version of the prior
- In practice, we are typically working in continous space
- In gerenal, the prior model can be arbitrarily complex
- There is a family of distributions, called exponential family, for which the prior has the same kernel as the posterior
- This is called conjugacy
- In practive, we seldom rely on conjugate relationships
...
- Recall that for the for continuos RVs we have, the CDF is defined as
...
- The above makes Beta a nantural choice for priors on the probability scale
...
- Recall, the definition of expectations for continous random variables
...
- It often more coninient to write the variance as
...
- Keeping in mind that a Gamma function is a continous analog of the factorial:

$$
\begin{eqnarray}
\E(\theta) &=& \int_{0}^{1} \frac{1}{\mathrm{B}(\alpha,\beta)} \, \theta \cdot \theta^{\alpha - 1} \, (1 - \theta)^{\beta - 1} \, d\theta \\
&=& \frac{1}{\mathrm{B}(\alpha,\beta)}\int_{0}^{1}  \, \theta^\alpha \, (1 - \theta)^{\beta - 1} \, d\theta \\
&=& \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) \, \Gamma(\beta)} \cdot 
\frac{\Gamma(1 + \alpha) \Gamma(\beta)}{\Gamma(1 + a + b)} \\
...
- We can find the the mode of this distribution by taking the log, differentiating with respect to $\theta$ and setting the derivative funtion to zero
...
- The variance of $\theta$ can derived using the definition of the variance operator
...
- Placenta previa is condition when the placenta completely or partially covers the opening of the uterus
- A PP study in Germany found that out of 980 births, 437 or $\approx 45\%$ were female
- Sex ratio in the population has been extremenly stable over space and time at 48.5% females with diviations within no more than 1%[^1]
...
$$
\begin{eqnarray}
\E(\theta \mid Y=y)  & = & \frac{\alpha + y}{\alpha + \beta + n} \\
\V(\theta \mid Y=y)  & = & \frac{(\alpha + y)(\beta + n - y)}{(\alpha + \beta + n)^2(\alpha + \beta + n + 1)} \\
\text{Mode}(\theta \mid Y=y) & = & \frac{\alpha + y - 1}{\alpha + \beta + n - 2} 
\end{eqnarray}
$$
...
```
- We can use the `quantile()` function to get the posterior interval and compute the event probability by evaluating the expectaion of the indicator function as before
...
::: {.incremental}
- What priors should we use if we think the sample that is draws from the sex ratio "hyperpopulation"?
...
## Bayesian Coherance
- A few examples of sequential updating
