...
::: incremental
- For positive integers $n$: $\, \Gamma(n+1) = n!$ and in general $\Gamma(z + 1) = z \Gamma(z)$
...
$$
\sigma_X^2 = \V(X) = \E(X - \mu)^2 = \int (x - \mu)^2 f(x) \, \text{d}x 
$$
...
&=& \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha) \, \Gamma(\beta)} \cdot 
 \color{red}{\frac{\Gamma(1 + \alpha) \Gamma(\beta)}{\Gamma(1 + \alpha + \beta)}} \\
&=& \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)} \cdot 
\frac{\Gamma(1 + \alpha)}{\Gamma(1 + \alpha + \beta)} \\
&=& \frac{\Gamma(\alpha + \beta)}{\Gamma(\alpha)} \cdot 
\frac{\alpha \Gamma(\alpha)}{(\alpha + \beta) \Gamma(\alpha + \beta)} \\
...
::: incremental
- The denominator is constant in $\theta$; we will derive the posterior up to proportionality
$$
\begin{eqnarray}
...
- As before, $\text{Mode}(\theta \mid y) = \E(\theta \mid y)$, when $\alpha = \beta$ and $y = n/2$
:::
...
f(\theta \mid y_1, y_2) &=&  \frac{f(\theta \mid y_1) f(y_2 \mid \theta)}{f(y_2 \mid y_1)} \\
&=& \frac{f(\theta)f(y_1 \mid \theta) f(y_2 \mid \theta)}{f(y_1)f(y_2 \mid y_1)}
\end{eqnarray}
$$
:::

::: incremental
- Observing data point $y_2$, and then data point $y_1$, will result in the same distribution
...
\begin{split}
f(\theta \mid y_1,y_2) 
& = \frac{f(\theta)f(y_1,y_2 \mid \theta)}{f(y_1, y_2)} \\
& = \frac{f(\theta)f(y_1 \mid \theta)f(y_2 \mid \theta)}{f(y_1, y_2)}
\end{split}
$$
:::
