...
Consider a sequence of random variables $Y_1, Y_2, Y_3, Y_4,...$, where for our purposes the subscripts index time. You can write the joint PDF as $f(y_1) f(y_2 \mid y_1) f(y_3 \mid y_2,y_1) f(y_4 \mid y_3, y_2, y_1)...$. Larry Wasserman uses a notational trick to write this as $\prod_{i=1}^{N} f(y_i \mid \text{past}_i)$, where $\text{past}_i = y_{i-1}, y_{i-2}, ..., y_0$
...
p +  geom_line(linewidth = 0.5) + scale_x_log10() + 
  ...
  ylab("Cumulative propotion of Up days") + xlab("Log10 Time") 
```

The process stars chaotic, but after about one thousand iterations, it seems to stabalize at around `r mean(y) |> round(2)`. We will now investigate this behavious analytically.
