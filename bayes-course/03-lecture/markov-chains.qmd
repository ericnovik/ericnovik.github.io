...
lin_trans <- function(M, x) {
  stopifnot(nrow(M) == ncol(M)) # square matrix
  stopifnot(ncol(M) == length(x))
  M %*% x
}
...
# Back to infinity and gambler's ruin

*This section is a stub: material on random walks on $\mathbb{Z}$, recurrence, and gambler's ruin can be added here.*
