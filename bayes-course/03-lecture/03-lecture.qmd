...
- Poisson RV $Y$ is parameterized with a rate of occurrence $\lambda$: $Y|\lambda \sim \text{Pois}(\lambda)$  
$$
f(y \mid \lambda) =  \frac{e^{-\lambda} \lambda^y}{y!}\;\; \text{ for } y \in \{0,1,2,\ldots\}
$$

- Notice, the Taylor series for $e^\lambda = \sum_{y=0}^{\infty} \frac{\lambda^y}{y!}$ immediately validates that $f$ is a PMF

- Also, $\E(Y | \lambda) = \V(Y | \lambda) = \lambda$, which is the restrictive case mentioned above --- real count data seldom satisfy this constraint
:::

## Visualizing Poisson {.smaller}
...
p + geom_line(linewidth = 0.2) + geom_point(size = 0.5)  +
  ggtitle("Number of soldiers in the Prussian cavalry killed by horse kicks") + ylab("") + xlab("")
```
:::
...
::: {.incremental}
- Assume we observe $Y_1, Y_2, ..., Y_n$ independent Poisson random variables
- The joint likelihood, a function of the parameter $\lambda$ for $y_i \in \mathbb{Z}^+$ and $\lambda > 0$, is given by:
$$
\begin{eqnarray}
...
- The Gamma PDF satisfies this condition: $f(\lambda) \propto \lambda^{\alpha - 1} e^{-\beta\lambda}$
...
## Prussian Army Horse Kicks {.smaller}
...
p1 <- dot_plot(x = deaths, y = actual, yc = pred, dodge = 0.1) +
  ...
  ggtitle("Death by horse-kick proportions", 
...
- The Normal PDF for one observation $y$ is given by:
$$
\begin{eqnarray}
...
- The grid approach to getting samples:
  1. Generate discrete points in parameter space $\theta$
...
- Here, $\theta$ is continuous, so we can not use the technique we used for a discrete, one-dimensional prior from the mystery box example.
...
$$
\begin{eqnarray}
f(\theta)f(y\mid \theta) &\propto& \theta^{\alpha - 1}(1 - \theta)^{\beta - 1} \cdot \prod_{i=1}^{n} \theta^{y_i} (1 - \theta)^{1 - y_i} \\
...
## Example 1
::: incremental
- Suppose we want to estimate the mean and variance of a RV
- In case of the mean, $h(\theta) := \theta$ and variance, $h(\theta) := \theta^2$
:::
...
\E[\theta^2] &=& \int_{-\infty}^{\infty} \theta^2 f(\theta)\, d\theta \\
&\approx& \frac{1}{N} \sum_{i = 1}^{N} \left( \theta^{(i)} \right)^2
\end{eqnarray}
$$
:::
...
# E(theta),   same as mean(theta)
(1/N * sum(theta)) |> round(2)   

# E(theta^2), same as mean(theta^2)
(1/N * sum(theta^2)) |> round(2) 
```
:::
...
#| echo: true
#| cache: true
pnorm(1, 0, 1) |> round(2)          # Evaluate N(0, 1) CDF at 1
N <- 1e5
...
- It produces dependent (autocorrelated) samples, but for a good algorithm, the dependence is manageable 
...
- It solves the problem of drawing from distribution $f(\theta)$ where $f$ is not one of the fundamental distributions and $\theta$ is high dimensional 
...
::: {.incremental}
- Stan is a procedural, statically typed, Turing complete, probabilistic programming language
...
$$
\mathbb{P}(X_{n+1}=j \mid X_0=i_0, X_1=i_1, \ldots, X_n=i_n) = \mathbb{P}(X_{n+1}=j \mid X_n=i_n)
$$
...
p2 <- mcmc_trace(bad_draws, pars = "mu") + ylab(expression(mu)) +
  ggtitle("Bad Chain")
grid.arrange(p1, p2, nrow = 2)
```
:::
...
bayesplot::rhat(bad_post, pars = "mu")  |> round(3)
```
:::
