...
- In practice, the challange is how to obtain draws from $f$?
...
- It produces dependant (autocorelated) samples but for a good algorithm, dependence is usually small
- Stan's MCMC algorithm is very efficient (more on that later) and requires all parameters to be continous (data can be discrete)
...
- What is high-dimentional? Modern algorithms like NUTS can jointy sample tens of thousands and more parameters
- That's 10,000+ dimentional integrals of complicated functions!
...
::: {.notes}
Include an example where h(x) is an indicator function and we want evaluate Normal CDF at some point t.
:::
...
- Will will address quality of the draws now and quality of predictions later in the course
:::
...
p1 <- mcmc_trace(draws, pars = "theta") + ylab(expression(theta)) +
  ggtitle("Good Chain")
bad_post <- readr::read_rds("data/bad_post.rds")
...
::: {.incremental}
- MCMC generates dependant draws from the target distribution
