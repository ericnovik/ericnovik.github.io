...
Ideally, discuss the project with your team, and agree on what needs to be done. We recommend one person to be the project manager and keep everyone on track. You can elect a PM or choose one randomly, or not at all -- it's up to you. We recommend that _every person_ performs a function of a tester of the final product -- read all the documents, run all the code, and discuss the results with the teammates.
...
Perform an exploratory data analysis and point out some interesting features in the data; plot important relationships. If the dataset is large, only summarize key features -- please, do not plot every variable, every two-way correlation, etc. Be selective and be nice to your readers.

### Statistical Model

Describe the statistical model. Use statistical notation to specify your likelihood and all the priors. If you have a complex model for the conditional mean, write it out explicitly. If you are not familiar with LaTex, see the LaTeX section.
...
Provide a short description of the limitations of your analysis, what worked and what didn't, and potential areas for further work

## Project Presentation

Prepare five slides summarizing your work: 1) Title with the name of the project and team members; 2) Research questions; 3) Statistical model; 4) Summary of results/predictions; 5) Conclusions.

You will only have 10 minutes to present, followed by a couple of questions from the audience. Don't cram every piece of information on every slide. Less is more. Here is an example of a good presentation.

## Elements of Style
You should develop a consistent style for presenting your work, including graphs and tables. Here are a few guidelines.

- Label all your graphs and axis and specify units if applicable.
...
- Don't use the default graph sizes, as they are generally too big (see the reference paper at the end of this section.)
- We recommend not using default points and line sizes, which are usually too large. You can use the `size` aesthetic in `geom_point` and `linewidth` aesthetic in `geom_line` to modify the defaults.
...
- If you are displaying uncertainty intervals, you should indicate the width of the interval (i.e., 50%, 90%, etc.) and the type of the interval (i.e., quantile, HDI). 50% quantile intervals are nice to have when you are plotting over observations, as the reader can quickly assess calibration (i.e., about half the points should be inside the interval and half outside). You don't have to report 95% uncertainty intervals, as those are arbitrary (as are others).
- If you want to show several plots side-by-side, use `gridExtra::grid.arrange` or something like that.

See, [this](https://www.mjskay.com/papers/infovis2020-uncertainty-decision-making.pdf) paper from Alex Kale, Matthew Kay, and Jessica Hullman as an example of some of the best practices in the field.
