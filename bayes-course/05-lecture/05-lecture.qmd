...
![](stan/sampler-exploration.gif){fig-align="center" height="680"}
...
$$
- Stan provides `log_sum_exp(...)`; in R it's a one-liner: `log_sum_exp <- function(z) { m <- max(z); m + log(sum(exp(z - m))) }`
...
fit_exp <- mod_exp$sample(
  data = stan_data, chains = 4, parallel_chains = 4,
  iter_sampling = 1000, refresh = 0, show_messages = FALSE,
  init = function() list(a = 1, b = 0.5, sigma = 1))
