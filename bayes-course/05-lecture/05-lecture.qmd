...
![](images/scared.avif){fig-align="center" width="672"}


[How was Homework 4?](https://www.polleverywhere.com/multiple_choice_polls/tW6UTzV6VKtaUk8yKO6jx)
...
1. In the case of linear regression, there is likely no causal mechanism and the coefficients should be interpreted as comparisons (RAOS, Page 84)
1. Set up reasonable priors and likelihood
1. (*) For more complex models: perform a forward simulation with fixed parameter values and try to recover them by running an inference algorithm. See [this](https://www.generable.com/post/fitting-a-basic-sir-model-in-stan) example for a standard epidemiological model.
1. Perform a prior predictive simulation
1. Adjust your priors
1. Fit the model to data
1. Assess the quality of the inference and the quality of the model
...
- If we work on the original scale for $x$, it is awkward to choose a prior for the intercept \alpha: it corresponds to the weight of the person with zero height
...
- Three dimentional posterior: $f(\alpha, \beta, \sigma)$. [What happened](https://www.polleverywhere.com/free_text_polls/Uj05I36LJbqhNiOylx6Wa) to $\mu$?
...
::: incremental
- A popular "proper" scoring for probabilistic prediction is log score
...
$$
\text{lppd} = \sum_{n=1}^{N} \frac{1}{S} \sum_{s=1}^{S} \log f(y_n \mid \theta_{-n, s})
$$
- To compute this quantity, fit the model N times, dropping one point at a time (-n)
- S is the number of posterior samples, and $\theta_{-n, s}$, is the s-th sample that we fit to model without the n-the data point
- [PSIS-LOO](https://arxiv.org/abs/1507.04544) (Pareto Smoothed Importance Sampling, Vehtari et al, 2017) provides a computationally efficient way to approximate LOO, without refitting the model N times
:::
