...
$$
- Stan provides `log_sum_exp(...)`; in R it's a one-liner: `log_sum_exp <- function(z) { m <- max(z); m + log(sum(exp(z - m))) }`. See [this](https://ericnovik.github.io/stats-math/02-lecture/02-lecture-read.html#/logarithmic-functions) for more details.
