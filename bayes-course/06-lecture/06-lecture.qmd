...
## Kalahari !Kung San people {.smaller}
```{r}
#| fig-width: 4.5
#| fig-height: 3.4
#| fig-align: center
...
- We will absorb $k$ into $\theta$, and so $w = \theta h^3$
:::

:::

::: {.column width="50%"}
![Robust fairing via conformal curvature flow, Crane et al. ACM Transactions on Graphics (2013)](images/Spot_the_cow.gif){fig-align="center" width="500"}
...
- We will absorb $k$ into $\theta$, and so $w = \theta h^3$, so the weight is proportional to the cube of height
:::
...
- This implies something like $\beta \sim \text{Normal}(3, 0.3)$
- We will leave our $\sigma \sim \text{Exponential}(1)$
...
transformed parameters {
  vector[N] mu = alpha + 3 * log_height;
}
model {
  alpha ~ normal(0, 5);
  sigma ~ exponential(1);
...
generated quantities {
  ...
  for (n in 1:N) {
    log_lik[n] = lognormal_lpdf(weight[n] | mu[n], sigma);
...
- If you have binary inputs, it may make sense to divide by two standard deviations (Page 186 in Regression and Other Stories)
...
p + geom_point(size = 0.5) + 
  geom_line(aes(y = lambda), 
            col = 'red', 
            linewidth = 0.2) +
  ggtitle("Simulated Poisson Data")
```
:::
...
## Checking Poisson Assumption {.smaller}
...
p + geom_point(size = 0.5) + 
ggtitle("Simulated Poisson Data with Exposure")
```
:::
...
slide_plot_theme <- theme_minimal(base_size = 6) +
  theme(
...
grid.arrange(
  p1 + slide_plot_theme,
  p2 + slide_plot_theme,
  p3 + slide_plot_theme + ggtitle("No Exposure"),
  p4 + slide_plot_theme + ggtitle("With Exposure"),
  ncol = 2
)
```
...
## Stan Model: Trapping Roaches {.smaller}

::: {style="font-size: 0.72em; line-height: 1.02;"}
```stan
data {
  int<lower=0> N;
...
model {
  ...
  if (!prior_only) {
    y ~ poisson_log(log_exposure + eta);
  }
}
generated quantities {
  array[N] int y_rep;
  for (n in 1:N) {
    y_rep[n] = poisson_log_rng(log_exposure[n] + eta[n]);
  }
}
```
::::
...
- The numbers are not in another universe, however, so we will go with it
