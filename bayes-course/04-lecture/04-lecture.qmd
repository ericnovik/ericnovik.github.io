...
- If accepted, set $\theta^{(2)} = \theta'$; otherwise stay at $\theta^{(2)} = \theta^{(1)}$
- Rinse and repeat
:::
...
::: {.column width="50%"}
::: {.incremental}
- If we can get an independent draw from $\theta$, we just take it
...
# proposal sampling distribution q = N(current_mean, proposal_scale)
  ...
  if (alpha > runif(1)) {         # this is just another way of flipping a coin
...
- Suppose you sample two points from the PDF $f(\theta \mid y)$, $\theta_a$ and $\theta_b$, and assume we are at time $t-1$
...
- $f(\tilde{y} \mid \theta, y) = f(\tilde{y} \mid \theta)$ since $y \perp\!\!\!\perp \tilde{y} \mid \theta$ (conditional independence)
...
## Introducing Posterior Predictive Distribution {.smaller}
::: {.incremental}
- For simple models we can often evaluate $\int f(\theta \mid y) f(\tilde{y} \mid \theta) \, d\theta$ directly
- We will use an indirect approach using our example Normal-Normal model with known variance
...
# Compiled binary is gitignored (see stan/.gitignore): always matches your installed CmdStan.
# If you ever see libtbb.dylib / "No chains finished" after a CmdStan upgrade, delete stan/normal_pred or pass force_recompile = TRUE.
m1 <- cmdstan_model("stan/normal_pred.stan")
