...
To call the Stan program from R, I will generate some data from the Binomial distribution with a known parameter \\( \theta = 0.7 \\) and ask Stan to recover this parameter. Incidentally this is a good general practice for starting the modeling process: create some fake data with known parameters and try to recover them. 
