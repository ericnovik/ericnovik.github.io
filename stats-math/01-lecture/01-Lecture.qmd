...
-   After completing the course, you will gain some experience with the **R** language, differential and integral **calculus**, basic **linear algebra**, and **probability**.
...
-   Each day will be broken up into three parts:
...
::: incremental
-   Introduction to the **R language**, RStudio, and R Markdown. Basic simulations.

-   Review of **differentiation**. Meaning of the derivative. Numeric and symbolic differentiation and optimization.

-   Review of **integration**. Riemann integral and basic rules of integration. Numeric and symbolic integration.

-   Review of **one-dimensional** **probability**. Conditional probability, random variables, and expectations. Some **discrete distributions**. Solving probability problems by simulation.
:::
...
-   Review of **Linear Algebra**. Vectors and vector arithmetic. Matrixes and matrix operations, multiplication, inverses, and determinants.

-   **Tentative: Two-dimentional probability** distributions (joint distributions). Joint, marginal, and conditional distributions. Covariance and correlation.
...
-   Programming is a cheap way to do experiments
-   Programming is easy; probabilistic programming is hard
-   Differential calculus: a way to optimize functions
-   Integral calculus: computing probabilities
-   Linear algebra: a language for n-dimensional problems
-   Probability: to statistics as Differential equations: to physics
:::
:::
...
-   We will learn about optimization later, but for now, imagine that we need to find the values of the slope and intercept such that the red line fits "nicely" through the cloud of points
:::
...
-   In the process, he [developed the scatterplot](https://friendly.github.io/HistDataVis/ch06-scat.html)
:::
...
dplyr::as_tibble(Virginis.interp)
```
:::
:::
...
-   Contrast with a data frame that contains objects of different types but is rectangular

```{r}
#| echo: true
x <- list(A = pi, B = c(0, 1), C = 1:10, D = c("one", "two"))
x$A
x[1]
str(x[1])
x[[1]] # same as x$A
...
#| fig-align: center
par(mar=c(3, 3, 2, 1), mgp = c(2, .7, 0), tck = -.01, bg = "#f0f1eb")
SnowMap(xlim=c(7.5,16.5), ylim=c(7,16), polygons=FALSE, density=TRUE,
...
sample(coin, 10, replace = TRUE, prob = c(0.7, 0.4))

# it's more convenient to make H = 1 and T = 0
coin <- c(1, 0)

# we can compute the proportion of heads in 1000 samples
sample(coin, 1e3, replace = TRUE) %>%
  mean()

# the above is the same as
mean(sample(coin, 1e3, replace = TRUE))
```
...
# flip a fair coin 2, 3, ..., 10000 times, and at each flip
# compute the proportion of heads
library(scales)
set.seed(123)
n <- 1e4
est <- numeric(n)
