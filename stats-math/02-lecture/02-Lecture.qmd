...
-   Your plan: climb the tower, drop the ball, and record the time it passes each **red mark** (spaced 1 m apart, 100 marks down)
...
::: incremental
-   There is no atmosphere on the Moon, so the only force on the ball is gravity. Newton's second law gives:
...
-   So the **acceleration** is the second derivative of position --- a constant $g$ --- which is exactly the unknown the mission needs.
:::

## The Drop, in Data {.smaller}
...
H <- 100                            # tower height in meters
marks  <- 1:H                       # red marks, every 1 meter
t_true <- sqrt(2 * marks / g)       # exact time to reach each mark
t_obs  <- t_true + rnorm(H, 0, 0.1) # noisy stopwatch reading at each flash

d <- data.frame(t = t_obs, x = marks)
p <- ggplot(d, aes(t, x)) + geom_point(size = 0.3) +
  xlab("time t (s)") + ylab("distance fallen x (m)") +
  ggtitle("Stopwatch reading at each light flash")
print(p)
```
:::
:::
...
::: {.column width="50%"}
::: {.fragment}
The inference problem (Session 6) is to recover $g$ from the noisy $(t_k, x_k)$ pairs, assuming $x(t) = \tfrac{1}{2} g t^2$. Here is the curve at the value we will recover, $g \approx 1.62$.
...
tt  <- seq(0, max(t_obs), length = 200)
sol <- data.frame(t = tt, x = 0.5 * g * tt^2)
p + geom_line(data = sol, aes(t, x), color = 'red', linewidth = 0.2) +
...
::: {.fragment}
**Oxygen is limited**, so each extra drop costs you. How few marks must we record to pin down $g$ to one decimal place? To two? We return to this in Session 7.
:::
...
$$
\begin{eqnarray}
v & = & \lim_{\Delta h \to 0} \frac{x(t + h) - x(t)}{h} =
\lim_{\Delta h \to 0} \frac{\tfrac{1}{2} g (t + h)^2 - \tfrac{1}{2} g t^2}{h} = \\
& \lim_{\Delta h \to 0} & \frac{g t h + \tfrac{1}{2} g h^2}{h} = \lim_{\Delta h \to 0} \left(g t + \tfrac{1}{2} g h\right) = g t
\end{eqnarray}
$$
:::

::: {.fragment}
- With our estimate $g \approx 1.62$, the speedometer at $t = 4$ reads $1.62 \cdot 4 \approx 6.5 \text{ m/s}$.
...
$$
a = \frac{dv}{dt} = \lim_{\Delta h \to 0} \frac{g(t + h) - g t}{h} =
\lim_{\Delta h \to 0} \frac{g h}{h} = \lim_{\Delta h \to 0} g = g
$$
:::
