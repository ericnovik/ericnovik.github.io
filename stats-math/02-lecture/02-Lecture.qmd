...
marks  <- seq(5, H, by = 5)         # 20 red marks, every 5 meters
t_true <- sqrt(2 * marks / g)       # exact time to reach each mark
n <- length(marks)
t_obs  <- t_true + rnorm(n, 0, 0.1) # noisy stopwatch reading at each flash
...
::: {.fragment}
**Oxygen is limited**, so each drop costs you. One drop gives 20 timed marks --- how many marks, and how many drops, do we need to pin down $g$ to one decimal place? To two? We return to this in Session 7.
:::
