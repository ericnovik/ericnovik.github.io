...
For this worked example, we are **not estimating $g$ yet**. We use the accepted gravity on Earth's Moon, $g_{\text{Moon}} = 1.625\ \text{m/s}^2$, together with a 0.30 s constant stopwatch delay and 0.10 s random timing noise. At that value of $g$, a 100 m drop takes about 11 s; on Earth it would take only 4.5 s.
...
timing_delay <- 0.30              # constant stopwatch delay, seconds
timing_sd <- 0.10                 # random stopwatch noise, seconds

x_marks <- seq(5, 100, by = 5)    # fixed, known distance marks
t_true <- sqrt(2 * x_marks / g_ref)

set.seed(12)
t_obs <- timing_delay + t_true +
  rnorm(length(x_marks), mean = 0, sd = timing_sd)
d <- data.frame(
  t = t_obs,
  x = x_marks
)

tt <- seq(0, max(t_true), length.out = 200)
sol <- data.frame(
  t = timing_delay + tt,
  x = 0.5 * g_ref * tt^2
)
...
p <- ggplot(d, aes(t, x)) +
  ...
  geom_point(size = 0.5) +
  annotate("text", 3, 80,
           label = "x(t) == frac(1, 2) * g * (t - alpha)^2",
           parse = TRUE, color = 'red') +
  xlab("observed time t (s)") + ylab("distance fallen x (m)") +
...
::: {.column width="42%"}
::: {.fragment}
- The red curve uses the accepted $g$ and the simulated delay $\alpha=0.30$ s; neither is estimated here. Later, we will recover both from data like these.
:::
