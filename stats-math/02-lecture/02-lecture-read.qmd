...
::: incremental
-   Let $x$ measure downward distance from the release point, so downward is positive. There is no atmosphere on the Moon, so the only force on the ball is gravity. Newton's second law gives:
...
g_ref <- 1.625                      # accepted lunar gravity, m/s^2

# One noisy time reading per known sensor position, collected in the browser app.
