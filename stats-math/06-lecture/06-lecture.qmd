...
t <- (0:50 * 11) / 50     # time points over an ~11 s fall
y <- 0.5 * g * t^2 + rnorm(length(t), 0, 3)
p <- ggplot(data.frame(t, y), aes(t, y))
p <- p + geom_point(size = 0.2) + xlab("time t (s)") + ylab("distance fallen x (m)") +
...
- $y$ is our outcome vector of length 51, capturing the distance fallen at each time point $t$
