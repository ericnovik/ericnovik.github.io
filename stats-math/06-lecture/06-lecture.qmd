...
- Model the observed time as
  $$
  t_i=\alpha+\sqrt{\frac{2x_i}{g}}+\varepsilon_i,
...
::: {.incremental style="font-size: 84%;"}
- Set $z_i=\sqrt{x_i}$ and $\beta=\sqrt{2/g}$ to obtain
  $$
  t_i=\alpha+\beta z_i+\varepsilon_i.
  $$

- With $\boldsymbol\theta=(\alpha,\beta)^\mathsf{T}$, the matrix model is
  $$
  \mathbf{t}=X\boldsymbol\theta+\boldsymbol\varepsilon,
  \qquad
  X=\begin{pmatrix}
  1 & \sqrt{x_1}\\
  \vdots & \vdots\\
...
::: {.fragment}
- The least-squares estimate is $\hat{\boldsymbol\theta}=(X^\mathsf{T}X)^{-1}X^\mathsf{T}\mathbf{t}$
:::
...
delay_hat <- as.numeric(theta_hat["intercept", ])
beta_hat <- as.numeric(theta_hat["sqrt_x", ])
g_hat <- 2 / beta_hat^2
...
::: {.fragment}
- $\hat\alpha=0.377\text{ s};\quad \hat g=1.691\text{ m/s}^2$.
:::
...
estimated_physical_curve <- transform(
  moon_curve,
  time_s = beta_hat * sqrt(distance_m)
)

fitted_observation_curve <- transform(
  moon_curve,
  time_s = delay_hat + beta_hat * sqrt(distance_m)
)
...
series_labels <- c(
  ...
  "observation fit: t̂ᵒᵇˢ(x) = α̂ + β̂√x"
)
...
ggplot() +
  ...
  labs(
    ...
    subtitle = paste(
      ...
      "the observation fit starts at α̂ = 0.377 s"
    )
  ) +
...
2. **Inverse and linear system.** Let $B=\begin{pmatrix}3&1\\2&1\end{pmatrix}$ and $\mathbf{b}=(7,5)^\mathsf{T}$. Compute $\det(B)$ and $B^{-1}$, solve $B\mathbf{x}=\mathbf{b}$ using $B^{-1}$, and verify the result with `solve()` in R.
