...
author: "Eric Novik | Summer 2023 | Session 6"
format:
  revealjs: 
    theme: ../../custom.scss
    scrollable: true
...
str_c("\\text{det}(A) = ") %>% str_c(tex(det(A))) %>% add_align() %>% cat()

B <- matrix(c("a", "b", "c", "d", "e", "f", "g", "h", "i"), 3, 3)
B <- as_sym(B)

str_c("B = ") %>% str_c(tex(B)) %>% add_align() %>% cat()
str_c("\\text{det}(B) = ") %>% str_c(tex(det(B))) %>% add_align() %>% cat()
...
str_c(tex(A)) %>% str_c("x = ") %>% str_c(tex(b)) %>% add_align() %>% cat()
str_c("\\text{det}(A) = ") %>% str_c(tex(det(A))) %>% add_align() %>% cat()
str_c("A^{-1} = ") %>% str_c(tex(inv(A))) %>% add_align() %>% cat()
...
- We are designing a Spam filter. Assume that 80% of email is spam. In 10% of spam emails, the word “free” is used, whereas this word is only used in 1% of non-spam emails. A new email has just arrived, which does contain the word "free." What is the probability that it is spam?

- Assume male and female heights follow a normal distribution with the following parameters:
...
```
- Generate realizations from the distribution of differences in heights
- Compute the P(male_h > femal_h) and compare it with the theoretical value
- Compute the variance and standard deviation of this distribution and compare them to the theoretical value
...
## New Homework {.smaller}

- For drawing 2-d vectors, you may find [these](https://gist.github.com/ericnovik/d07c883545f98b336f3855a2bab87421) functions helpful
...
w <- c(3, 1)
draw_vector(v, col = 'red') %>%
  add_vector(w, col = 'blue', linetype = "dashed")
```

- Create two new vectors v = [-1, 3] and w = [2, 1]
- Apply the clockwise rotation matrix to each and plot their original and final positions
- Apply the counterclockwise rotation matrix to the result and convince yourself that the vectors are back at v = [-1, 3] and w = [2, 1]
...
\end{eqnarray}
$$
- Compute the determinant. Are the columns linearly independent?
- Solve the system and report the results
- Perform the following analysis:

```{r}
#| echo: true

# install this package: install.packages('foreign')
library(foreign)

# dowlaod a dataset:
iq <- read.dta("http://www.stat.columbia.edu/~gelman/arm/examples/child.iq/kidiq.dta")

# dataset contains IQs of moms and their kids (kid_score)
head(iq)
```

- Plot the relationship between `mom_iq` and `kid_score` (make `kid_score` the output)
- What can you tell from the plot?
- Now color by `mom_hs` (1 means mom completed high school). What can you tell?
- Compute the regression line (by any method) for `kid_score ~ mom_iq`
- Plot the regression line over the data. Does it fit?
- Now compute the regression conditional on mom completing high school (i.e., only look at records where `mom_hs = 1`)
- Do the same, but now for `mom_hs = 0`
- Plot both regression lines (in different colors) over the observations. Do these regression lines make sense? 
