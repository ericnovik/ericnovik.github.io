...
::: incremental
- Recall, from Session 2, the Moon mission: we drop a ball from a tower and record the distance fallen at noisy time points to infer the Moon's gravity $g$
- At the time, we promised we would later learn how to find $g$ from the data
...
marks <- seq(5, 100, by = 5)     # 20 red marks, every 5 m down the tower
t <- sqrt(2 * marks / g)         # exact pass time at each mark
y <- marks + rnorm(length(marks), 0, 0.8)   # noisy distance reading at each flash
p <- ggplot(data.frame(t, y), aes(t, y))
p <- p + geom_point(size = 0.5) + xlab("time t (s)") + ylab("distance fallen x (m)") +
  ggtitle("Noisy measurements of the ball's fall")
print(p)
```
:::
...
- The equation of motion is $x(t) = \tfrac{1}{2} g t^2$, which we will write as $y(t) = \beta_0 + \beta_1 t^2$

- We keep an intercept $\beta_0$ as a check: since the ball is dropped from rest at $x = 0$, we expect $\hat{\beta}_0 \approx 0$, while $\hat{\beta}_1 \approx g/2$, so $\hat{g} = 2\hat{\beta}_1$

- Let's express it in the matrix notation

- What is our design matrix $X$? We have 20 observations (one per mark), so $X$ will have two columns and 20 rows: a column of 1s for the intercept and a column of times squared $t^2$

- Our unknown vector $\hat{\beta}$ has two elements $(\hat{\beta_0},\, \hat{\beta_1)}$

- $y$ is our outcome vector of length 20, capturing the distance fallen at each time point $t$
...
# intercept is ~0 (dropped from rest); g = 2 * beta_1
g_hat <- 2 * beta_hat[2]
round(g_hat, 3)
...
::: {.fragment}
- We recover $\hat{g} = 2\hat{\beta}_1 \approx 1.62$ --- the mission is a go. (Warning: do not use $(X^TX)^{-1}X^Ty$ directly as above; it's computationally inefficient.)
:::
...
#| fig-align: center
#| echo: true
d <- data.frame(t = t, yhat = beta_hat[1] + beta_hat[2] * t^2)
