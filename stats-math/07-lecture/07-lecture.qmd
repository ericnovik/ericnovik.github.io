...
## Moon Drop: How Many Marks? {.smaller}

Oxygen is limited, so each drop costs you. How many marks must we record to pin down $g$? We simulate: for each $n$, fit $\hat{g} = 2\hat{\beta}_1$ many times and measure the spread.
...
set.seed(1)
g_true <- 1.625
sigma  <- 0.5            # distance-reading noise, meters
reps   <- 500

se_ghat <- function(n_marks) {
  marks <- 1:n_marks
  tsq   <- (sqrt(2 * marks / g_true))^2          # squared pass times
  ghat  <- replicate(reps, {
    y <- 0.5 * g_true * tsq + rnorm(n_marks, 0, sigma)
    2 * coef(lm(y ~ 0 + tsq))[1]                 # g = 2 * slope
  })
  sd(ghat)                                       # empirical SE of g_hat
}

ns <- c(5, 10, 15, 20, 40, 70, 100)
se <- sapply(ns, se_ghat)
data.frame(n_marks = ns, se = round(se, 4),
           ci_half_width = round(1.96 * se, 4))
```
:::

::: {.fragment}
- **One decimal** ($\pm 0.05$): a CI half-width under $0.05$ needs only ~15 marks
- **Two decimals** ($\pm 0.005$): you need ~70 marks --- most of the tower
- Precision costs data: the second decimal is ~5× more expensive than the first
:::
