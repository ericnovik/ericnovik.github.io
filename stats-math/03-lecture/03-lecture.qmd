...
author: "Eric.Novik@nyu.edu | Summer 2025 | Session 3"
format:
  revealjs: 
    theme: ../../custom.scss
    scrollable: true
...
- And [here](https://gist.github.com/ericnovik/9ac7feb95ed89928faf3fa71b45eecdb) is much shorter version of the exercise.
...
::: incremental
-   Unknowns are sometimes called parameters, like the Moon's gravity $g$ in our drop model $x(t) = \tfrac{1}{2} g t^2$
:::
:::
:::
...
::: incremental
-   Recall the Moon drop: from $\ddot{x} = g$ we found the velocity $v(t) = g t$
-   This is the promised derivation: integrate $v(t)$ once to *recover the position function* $x(t)$
:::
...
x(t) & = & \int{(g t + C_1)\, dt} = \tfrac{1}{2} g t^2 + C_1 t + C_2
\end{eqnarray}
$$
:::

::: incremental
-   The ball is *dropped from rest* at the top of the tower, so the initial velocity is zero ($C_1 = 0$), and we measure distance fallen from the release point, so the initial position is zero ($C_2 = 0$).

-   That leaves $x(t) = \tfrac{1}{2} g t^2$. In general, to characterize the position function you need the initial position and velocity --- you can't get those from the acceleration alone.
:::

## Plot

The position function for different values of initial position $C_2$. Notice that the only thing that changes is the intercept.
...
p + geom_line(aes(color = name)) +
  ...
  ggtitle("Position functions for different initial positions") +
  annotate("text", 2, 180, label = "x(t) == frac(1, 2) * g * t^2 + C[2]", parse = TRUE) +
  guides(color = guide_legend(title = expression(C[2])))
